...
Hexapod :: Hexapod(){
	...
	driver1 -> initializeServoDriver(60);
	driver2 -> initializeServoDriver(60);
	...
	for(int i = 0; i < 18; i++){
		...
		if(i < 9){
			driver1 -> setPWMValue(i, pwmValue);
		}
		else{
			driver2 -> setPWMValue(i - 9, pwmValue);
		}
...
void Hexapod :: walkSide(int numOfSteps){
...
void Hexapod :: turn(int numOfSteps){
...
void Hexapod :: walkCombined(float angle, int numOfSteps){
